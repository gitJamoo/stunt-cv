...
- **`server.py` + `web/index.html`** — FastAPI local web UI (the planned successor to the Tk app — see `TODO.md`). JSON API: `/api/videos`, `/api/upload` (file-explorer upload into `raw_videos/`), `/api/analyze` (background job, one at a time — `PoseTracker` is not thread-safe; cancellable), `/api/jobs/{id}` (polling), `/api/analysis/{video}` (cached result from `analyses/<video>.json`), `/api/frame/{video}/{i}` (JPEG frames for the canvas player — skeletons are drawn client-side from per-frame poses in the analysis result, so the JS `CONN` list must match `pose_data.YOLO_CONNECTIONS`), `/api/chat` (DeepSeek proxy; browser-supplied key or `DEEPSEEK_API_KEY` env fallback). Binds localhost, no auth.
